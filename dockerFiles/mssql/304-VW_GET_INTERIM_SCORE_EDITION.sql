...
WITH domesticLeagues AS (
SELECT COMPETITION_NAME 
FROM COMPETITION
WHERE COMPETITION_TYPE = 'Nationale Kampioenschap'
)
, summedStats AS (
SELECT SEASON_NAME,
		COMPETITION_NAME,
...
FROM GOALS_CLUB_MATCH g
WHERE EXISTS (SELECT 1 FROM domesticLeagues d where d.COMPETITION_NAME = g.COMPETITION_NAME)
GROUP BY COMPETITION_NAME, SEASON_NAME, CLUB_NAME) -- tot goals
, winsClubEdition AS (
SELECT SEASON_NAME, COMPETITION_NAME, CLUB_NAME, count(*) as amount
FROM GOALS_CLUB_MATCH g
WHERE [type] = 'W' AND EXISTS (SELECT 1 FROM domesticLeagues d where d.COMPETITION_NAME = g.COMPETITION_NAME)
GROUP BY COMPETITION_NAME, SEASON_NAME, CLUB_NAME
)
, drawsClubEdition AS (
SELECT SEASON_NAME, COMPETITION_NAME, CLUB_NAME, count(*) as amount
FROM GOALS_CLUB_MATCH g
WHERE [type] = 'G' AND EXISTS (SELECT 1 FROM domesticLeagues d where d.COMPETITION_NAME = g.COMPETITION_NAME)
GROUP BY COMPETITION_NAME, SEASON_NAME, CLUB_NAME
)
, unranked_score AS (
