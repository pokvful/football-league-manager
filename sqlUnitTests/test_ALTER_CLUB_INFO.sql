...
BEGIN
	...
	DROP TABLE IF EXISTS test_ADD_PLAYER_CLUB.expected

	DROP TABLE IF EXISTS test_ADD_PLAYER_CLUB.expected2
END
GO
...
BEGIN
	SELECT TOP(0) * 
	INTO test_ADD_PLAYER_CLUB.expected
	FROM PERSON

	SELECT TOP(0) *
	INTO test_ADD_PLAYER_CLUB.expected2
	FROM PLAYER

	SET IDENTITY_INSERT test_ADD_PLAYER_CLUB.expected ON
	INSERT INTO test_ADD_PLAYER_CLUB.expected (PERSON_ID, COUNTRY_NAME, FIRST_NAME, LAST_NAME, MIDDLE_NAME, BIRTH_DATE)
	VALUES (1, 'United Kingdom', 'first', 'last', null, '2000-01-01')

	INSERT INTO test_ADD_PLAYER_CLUB.expected2 (PERSON_ID, CLUB_NAME, JERSEY)
	VALUES (1, 'club1', 22)

	EXEC dbo.ADD_PLAYER_CLUB 'first', 'last', '2000-01-01', 'United Kingdom', 'club1', 22

	EXEC tSQLt.AssertEqualsTable 'test_ADD_PLAYER_CLUB.expected', 'PERSON'
	EXEC tSQLt.AssertEqualsTable 'test_ADD_PLAYER_CLUB.expected2', 'PLAYER'
END
GO
...
BEGIN
	...
	DROP TABLE IF EXISTS test_ALTER_PLAYER_CLUB.expected
END
GO
...
BEGIN
	SELECT TOP(0) *
	INTO test_ALTER_PLAYER_CLUB.expected
	FROM PLAYER

	INSERT INTO test_ALTER_PLAYER_CLUB.expected (PERSON_ID, CLUB_NAME, JERSEY)
	VALUES (1, 'ander', 44)

	EXEC ALTER_PLAYER_CLUB 1, 'ander'

	EXEC tSQLt.AssertEqualsTable 'test_ALTER_PLAYER_CLUB.expected', 'PLAYER'
END
GO
...
BEGIN
	...
	DROP TABLE IF EXISTS test_UPDATE_CLUB.expected
	DROP TABLE IF EXISTS test_UPDATE_CLUB.expected2
...
BEGIN
	SELECT TOP(0) * 
	INTO test_UPDATE_CLUB.expected
	FROM CLUB

	SELECT TOP(0) *
	INTO test_UPDATE_CLUB.expected2
	FROM STADIUM

	INSERT INTO test_UPDATE_CLUB.expected (CLUB_NAME, STADIUM_NAME, COACH_PERSON_ID, COUNTRY_NAME, CITY_NAME)
	VALUES ('changedClub', 'stadium2', 4, 'cc', 'ccc')

	INSERT INTO test_UPDATE_CLUB.expected2 (STADIUM_NAME, CAPACITY)
	VALUES ('stadium', 666), ('stadium2', 333)
	...
	EXEC tSQLt.AssertEqualsTable 'test_UPDATE_CLUB.expected', 'CLUB'
	EXEC tSQLt.AssertEqualsTable 'test_UPDATE_CLUB.expected2', 'STADIUM'
END
GO
