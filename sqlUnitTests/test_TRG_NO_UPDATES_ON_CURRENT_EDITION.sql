...
CREATE OR ALTER PROCEDURE [test_IR16_CurrentEditionChanges].[Test that checks if changes get prevented in MATCH during a current edition SUCCES CURRENT SEASON and COMPETITION_TYPE ko]
AS
BEGIN
	EXEC tSQLt.FakeTable @TableName='dbo.MATCH';
	EXEC tSQLt.FakeTable @TableName='dbo.MATCHDAY';
...
CREATE OR ALTER PROCEDURE [test_IR16_CurrentEditionChanges].[Test that checks if changes get not prevented in COMPETITION during a current edition PASSING COMPETITION_TYPE Knockout]
AS
BEGIN
	EXEC tSQLt.FakeTable @TableName='dbo.COMPETITION'
	EXEC tSQLt.FakeTable @TableName='dbo.EDITION';
